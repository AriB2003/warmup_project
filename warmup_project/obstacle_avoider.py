...
class ObstacleAvoiderNode(Node):
    ...
    def run_loop(self):
        ...
        if (self.scan_results is not None):
            ...
            # Generate vectors from all relevant points (from -35 to 35
            # degrees)
            for i in range(35):
                current_vector = calculate_potential_vector(
                    i, self.scan_results[i])
...
def calculate_potential_vector(angle, distance):
    ...
    # points in the middle should be more heavily weighted
    relative_weight = min((-angle_off_heading**2 + 2025), 1800)
    ...
    potential_vector = [
        ...
        r *
        math.sin(
            math.radians(theta))]  # [x,y] - x is forward and y is left
